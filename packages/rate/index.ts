...
LinComponent({
  field: true,
  classes: ["icon-class", "void-icon-class"],
  props: {
    // 图标间距，默认单位为 px
    gutter: {
      type: [String, Number],
  ...
  methods: {
    ...
    setCount() {
      this.setData({
        innerCountArray: Array.from({ length: this.data.count })
      });
    },
    // 设置激活的宽度
    setWidth() {
      let { value, gutter } = this.data;
    ...
    onClick(event: WechatMiniprogram.TouchEvent) {
      const { allowHalf, readonly, disabled } = this.data;
