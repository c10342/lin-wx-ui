...
const TransitionBehavior = (showDefaultValue?: boolean) =>
  Behavior({
    properties: {
      // 自定义样式
      customStyle: String,
    ...
    methods: {
      ...
      enter() {
        ...
        Promise.resolve()
          ...
          .then(() => {
            ...
            this.checkStatus("enter");
            // 标志位，标志过渡动画是否结束了
            (this as any).transitionEnded = false;
      ...
      onTransitionEnd() {
        ...
        // 标记过渡动画结束
        (this as any).transitionEnded = true;
